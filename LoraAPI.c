...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
LoraInitRx(): Initilizes LoRa to Tx mode (RX FIFO ptr, TX highpower mode, HighFreq), output PIN mapping, and puts tranciver into sleep mode
Inputs:
- n/a
Outputs:
- (bool) successfull or not
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
LoraGetMessage(): sets configuration register to 
Inputs:
- n/a
Outputs:
- (uint8_t) size of message (used for looking through storage array)
Notes:
- If output is 0, an error has occoured (ie. Rx Timeout, CRC error...) OR other error within Rx retrevial 
- Will not effect mode of LoRa tranciver, it will stay in whatever mode it is in
Issues:
- keep in mind, a 0 means error ANYWHERE in the code
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
uint8_t LoraGetMessage(void){
	uint8_t numBytesReceived = 0x00;
	uint8_t storageLocation = 0x00;
	uint8_t RX_FifoPtr = 0x00;
	int i = 0;
	uint8_t Register0x12 = loRaRead(0x12);
	if((Register0x12 & 0x80) == 0x80 ||(Register0x12 & 0x20) == 0x20){
		//0x80 = RX timeout
		//0x50 = CRC error
		...
		loRaWrite(0x12,0xFF);
		return 0;
	}else if((Register0x12 & 0x40)== 0x40 && (Register0x12 & 0x10) == 0x10){
		
		//clear flag
		loRaWrite(0x12,0xFF);
		if(loRaRead(0x12) != 0x00){
			return 0;//error clearing
		}
		
		//extract data info
		numBytesReceived = loRaRead(0x13);
		storageLocation = loRaRead(0x10);
		
		//set FifoPtr to storage location
		loRaWrite(0x0D, storageLocation);
		
		//extract data
		for(i = 0; i < numBytesReceived	; i++){
			storageArray[i] = loRaRead(0x00);			
		}
		
		//reset FIFO ptr
		RX_FifoPtr = loRaRead(0x0F); //Rx FIFO base addrss
		loRaWrite(0x0D,RX_FifoPtr);//reset value
		if(loRaRead(0x0D) != RX_FifoPtr){
			return 0;
		}

		return numBytesReceived;
	}else{
		//not error nor valid messsage, was this called in error?
		return 0;
	}
}
